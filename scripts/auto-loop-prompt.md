...
### 작업 크기 정책
열린 이슈가 없어 신규 작업을 직접 만들 때는 토큰/리뷰 고정비를 감안해 너무 작은 단위로
쪼개지 않는다. 기본 목표는 **중간 크기 vertical slice**다.

- 하나의 명확한 도메인 경계 또는 primary plan의 한 섹션만 다룬다.
- 관련 acceptance test 3-6개를 포함한다.
- 예상 변경 파일은 2-5개로 유지한다.
- 작은 단일 helper/API 작업은 같은 데이터 흐름의 인접 단계와 묶는다.
- 서로 다른 도메인, PR/루프 인프라 변경, 대규모 리팩터는 기능 작업과 묶지 않는다.
- 제안과 구현 brief에는 반드시 Non-goals와 Stop conditions를 적는다.

그 다음:
- 계획서 작성: **{무엇/왜} + 작성할 테스트 케이스(TDD, pytest/hypothesis) + 변경 파일 예상 + 연결 이슈(#번호 또는 "신규") + Non-goals + Stop conditions**.
...
현재 `review_cycle`의 `status`가 이미 `HOLD` 또는 `NEEDS_REVISION`이고 그 이후
새 Codex 구현/리뷰 사이클 산출물이 없으면, 같은 cycle을 다시 리뷰하거나 PR 승인 gate를
열지 않는다. 상태를 그대로 보존하고 새 Codex 수정 사이클을 기다린다.
...
3. P1이 없으면 멘션 없는 리뷰 결과와 PR 승인 문구를 Discord에 보내고, 반환 메시지 ID를
   ...
   `REVIEW-*-CLAUDE-*.md`에 `status: APPROVED`를 썼더라도 이 상태 전이까지 끝내기 전에는
   Claude 리뷰가 완료된 것이 아니다.
