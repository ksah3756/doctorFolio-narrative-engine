...
def test_each_calibrated_chunk_requires_two_schema_valid_repeats() -> None:
    responses = [
        _response(
            "chunk-a",
            _claim(
                claim_id="a-1",
                text="Cloud demand accelerated.",
                subject="DEMAND_SIGNAL",
                direction="INCREASE",
            ),
        ),
    ...
    with pytest.raises(ValueError, match="at least two schema-valid repeats per chunk"):
        calibrate_extraction_replay(responses)


def test_duplicate_claim_groups_do_not_push_agreement_above_one() -> None:
    responses = [
        _response(
            "chunk-1",
            _claim(
                claim_id="claim-1a",
                text="AI infrastructure demand increased sharply.",
                subject="DEMAND_SIGNAL",
                direction="INCREASE",
            ),
            _claim(
...
def test_duplicate_claim_groups_do_not_hide_direction_drift() -> None:
    responses = [
        _response(
            "chunk-1",
            _claim(
    ...
    assert len(result.unstable_groups) == 1
    unstable = result.unstable_groups[0]
    assert unstable.agreement_rate == 0.5
    assert unstable.label_counts == {
        ("COST_SIGNAL", "INCREASE"): 1,
        ("COST_SIGNAL", "DECREASE"): 1,
    }
...
def _response(chunk_id: str, *claims: Claim) -> ExtractionResponse:
